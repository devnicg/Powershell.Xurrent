function Get-XurrentGraphQLOrganization {
    <#
    .SYNOPSIS
        Retrieves one or more Xurrent organizations via the GraphQL API.
    .PARAMETER Id
        The node ID of a specific organization to retrieve.
    .PARAMETER Filter
        A hashtable of filter parameters.
    .PARAMETER Fields
        The GraphQL fields to return. Defaults to 'id name'.
    .PARAMETER First
    ...
    process {
        if ($PSCmdlet.ParameterSetName -eq 'ById') {
            $query = "query(`$id: ID!) { organization(id: `$id) { $Fields } }"
            ...
            $query = @"
query($varDefs) {
    organizations(first: `$first, after: `$after$filterArgs) {
        nodes { $Fields }
