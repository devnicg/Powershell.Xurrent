function Get-XurrentGraphQLService {
    <#
    .SYNOPSIS
        Retrieves one or more Xurrent services via the GraphQL API.
    .PARAMETER Id
        The node ID of a specific service to retrieve.
    .PARAMETER Filter
    ...
    process {
        if ($PSCmdlet.ParameterSetName -eq 'ById') {
            $query = "query(`$id: ID!) { service(id: `$id) { $Fields } }"
            ...
            $query = @"
query($varDefs) {
    services(first: `$first, after: `$after$filterArgs) {
        nodes { $Fields }
