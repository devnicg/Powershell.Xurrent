function Get-XurrentGraphQLServiceInstance {
    <#
    .SYNOPSIS
        Retrieves one or more Xurrent service instances via the GraphQL API.
    .PARAMETER Id
        The node ID of a specific service instance to retrieve.
    .PARAMETER Filter
        A hashtable of filter parameters.
    .PARAMETER Fields
        The GraphQL fields to return. Defaults to 'id name status'.
    .PARAMETER First
    ...
    process {
        if ($PSCmdlet.ParameterSetName -eq 'ById') {
            $query = "query(`$id: ID!) { serviceInstance(id: `$id) { $Fields } }"
            ...
            $query = @"
query($varDefs) {
    serviceInstances(first: `$first, after: `$after$filterArgs) {
        nodes { $Fields }
