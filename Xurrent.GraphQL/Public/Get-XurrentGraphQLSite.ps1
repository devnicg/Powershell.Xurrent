function Get-XurrentGraphQLSite {
    <#
    .SYNOPSIS
        Retrieves one or more Xurrent sites via the GraphQL API.
    .PARAMETER Id
        The node ID of a specific site to retrieve.
    .PARAMETER Filter
    ...
    process {
        if ($PSCmdlet.ParameterSetName -eq 'ById') {
            $query = "query(`$id: ID!) { site(id: `$id) { $Fields } }"
            ...
            $query = @"
query($varDefs) {
    sites(first: `$first, after: `$after$filterArgs) {
        nodes { $Fields }
