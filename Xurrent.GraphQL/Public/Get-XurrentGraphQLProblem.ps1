function Get-XurrentGraphQLProblem {
    <#
    .SYNOPSIS
        Retrieves one or more Xurrent problems via the GraphQL API.
    .PARAMETER Id
        The node ID of a specific problem to retrieve.
    .PARAMETER Filter
        A hashtable of filter parameters.
    .PARAMETER Fields
        The GraphQL fields to return. Defaults to 'id subject status impact'.
    .PARAMETER First
    ...
    param (
        ...
        [Parameter()]
        [string] $Fields = 'id subject status impact',
    ...
    process {
        if ($PSCmdlet.ParameterSetName -eq 'ById') {
            $query = "query(`$id: ID!) { problem(id: `$id) { $Fields } }"
            ...
            $query = @"
query($varDefs) {
    problems(first: `$first, after: `$after$filterArgs) {
        nodes { $Fields }
