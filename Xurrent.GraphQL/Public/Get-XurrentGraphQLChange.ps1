function Get-XurrentGraphQLChange {
    <#
    .SYNOPSIS
        Retrieves one or more Xurrent changes via the GraphQL API.
    .PARAMETER Id
        The node ID of a specific change to retrieve.
    .PARAMETER Filter
        A hashtable of filter parameters.
    .PARAMETER Fields
        The GraphQL fields to return. Defaults to 'id subject status category'.
    .PARAMETER First
    ...
    param (
        ...
        [Parameter()]
        [string] $Fields = 'id subject status category',
    ...
    process {
        if ($PSCmdlet.ParameterSetName -eq 'ById') {
            $query = "query(`$id: ID!) { change(id: `$id) { $Fields } }"
            ...
            $query = @"
query($varDefs) {
    changes(first: `$first, after: `$after$filterArgs) {
        nodes { $Fields }
