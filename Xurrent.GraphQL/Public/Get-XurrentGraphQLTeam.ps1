function Get-XurrentGraphQLTeam {
    <#
    .SYNOPSIS
        Retrieves one or more Xurrent teams via the GraphQL API.
    .PARAMETER Id
        The node ID of a specific team to retrieve.
    .PARAMETER Filter
    ...
    process {
        if ($PSCmdlet.ParameterSetName -eq 'ById') {
            $query = "query(`$id: ID!) { team(id: `$id) { $Fields } }"
            ...
            $query = @"
query($varDefs) {
    teams(first: `$first, after: `$after$filterArgs) {
        nodes { $Fields }
